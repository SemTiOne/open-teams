...
完整操作见 [快速上手](./QUICKSTART.md)。最短路径只需要三步：

1. 复制本仓库作为你的业务工作空间。
2. 修改 `workspace-config/code-sources.yaml`，填入真实项目仓库或源码目录。
3. 运行 `python3 scripts/validate_template_layout.py .`，确认模板结构完整。

接下来按需要逐步补齐：

- 调整 `AGENTS.md`，写入团队自己的 AI 协作边界。
- 在 `docs/projects/<project>/` 记录首批项目说明。
- 用 `scripts/init_project_skills.py <template_root> <project_slug>` 初始化项目场景 skill。
- 从 `workspace-assets-index.md` 进入后续文档、skill 和参考资料。
