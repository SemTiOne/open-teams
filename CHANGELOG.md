...
- 强化节点级版本维护门禁：任务开始时必须确认工作空间和相关 `sources` 仓库的分支、远端跟踪和工作区状态。
- workflow skills、任务计划模板和升级提示词同步要求：每个实施节点经用户验收通过后，必须先提交并推送该节点相关改动，再进入下一个节点。
- 提交推送完成后，AI 必须说明提交范围、远端分支、提交结果，并给出包含具体任务或节点名称的 `你可以说：` 下一步推荐动作。

### 升级影响

- 多节点任务的版本维护更细粒度，避免多个已验收节点长期堆积到最终一次性提交。
- 业务工作空间如存在多个 `sources` 源码仓库，应在任务开始和节点验收后分别记录适用性；不涉及源码仓库时应明确标注不适用。

### 兼容性

- 不涉及业务代码或接口行为变化。
- 对既有业务工作空间的主要影响是 AI 协作流程更严格，适合作为治理规则补丁按需应用。
